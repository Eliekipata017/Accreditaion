...
export const DashboardGeneral =  ()  => {
    const userId = localStorage.getItem("dirGUser")
    ...
    const fetchData = async ()=>{
        const demandes = await axios.post('http://localhost:3000/getDemandesAccreditaions')
        console.log(demandes)
        const user = await axios.post(`http://localhost:3000/getDirecteur`, {id: parseInt(userId)})
    ...
    useEffect(() => {
        ...
        if (dmd){
            ...
            const DnonEncours = dmd.filter((dm)=> dm.statut == 3)
            const DFinis = dmd.filter((dm)=> dm.statut == 4)
